...
async def main():
    ...
    selection_role_description = """You are a patch selection agent. Your job is to select the BEST candidate patch for a bug in Java code.

You will receive several candidate patches. Each candidate was generated by a different patching agent (identified by its patcher id) and has already PASSED all test suites, so correctness on the existing tests is not a differentiator.
...
- Following lines: a brief explanation of why you chose it over the others"""
    ...
    try:
        ...
        if selection_response.selected_patch:
            print(f"Selected patch: {selection_response.selected_patch}")
            print(f"Final patch location: {bug_dict.get_info('final patch path')}")
        else:
            print("No candidate patches passed the test suites, so no final patch was selected.")
    finally:
